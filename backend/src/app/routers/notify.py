...
@router.post("/block-alert")
async def block_alert(
    payload: BlockAlertPayload,
    current_user: dict = Depends(get_current_user)
):
    """
    Send an immediate alert to a parent when a dangerous page is blocked.
    Looks up the parent via the child's username and emails the parent.
    """
    try:
        async with async_session() as session:
            # Find child by username
            child_q = await session.execute(
                select(users).where(users.c.username == payload.child_username, users.c.account_type == "child")
            )
            child_row = child_q.fetchone()
            if not child_row:
                return {"success": False, "reason": "no_parent_email"}

            # Lookup parent
            parent_q = await session.execute(
                select(users).where(users.c.id == child_row.parent_id)
            )
            parent_row = parent_q.fetchone()
            if not parent_row or not parent_row.email:
                logger.warning(f"[Notify] No parent email for child {payload.child_username}")
                return {"success": False, "reason": "no_parent_email"}

        subject = "⚠️ GuardianLens Alert — Dangerous Page Blocked"
        # Simple clean HTML body
        html_body = f"""
        <div style="font-family:system-ui,-apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;max-width:600px;margin:0 auto;padding:20px;background:#f8fafc;border-radius:8px">
          <h2 style="margin:0 0 12px;color:#0f172a">⚠️ GuardianLens Alert</h2>
          <p style="margin:0 0 8px;color:#334155">Child: <strong>{payload.child_username}</strong></p>
          <p style="margin:0 0 8px;color:#334155">Blocked URL: <a href="{payload.blocked_url}">{payload.blocked_url}</a></p>
          <p style="margin:0 0 8px;color:#334155">Reason: {payload.reason}</p>
          <p style="margin:0 0 8px;color:#64748b">Timestamp: {payload.timestamp}</p>
          <hr style="border:none;border-top:1px solid #e2e8f0;margin:16px 0">
          <p style="font-size:12px;color:#94a3b8;margin:0">This alert was generated by GuardianLens — the report is private and was not shown to your child.</p>
        </div>
        """

        sent = send_email(parent_row.email, subject, html_body)
        if not sent:
            logger.error(f"[Notify] Failed to send block-alert to {parent_row.email}")
        ...
        logger.exception("Failed to process block-alert")
