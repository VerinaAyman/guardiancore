...
try:
    ...
    LENS_ROUTER_AVAILABLE = True
except Exception as e:
    lens_router = None
    LENS_ROUTER_AVAILABLE = False
    logging.getLogger(__name__).warning(f"Lens router disabled at startup: {e}")
...
async def startup_event():
    ...
    try:
        ...
        if settings.ENV != "dev":
            raise
        logger.warning("Running without DB in dev mode; DB-backed endpoints may fail")
