...
async def analyze_content(
    ...
    try:
        ...
        if not result['safe'] and result.get('blocked_by') == 'domain_category':
            return ContentAnalysisResponse(
                safe=False,
                action='warn',
                blocked_by='domain_category',
        ...
        should_run_pipeline = bool(text_content)

        if should_run_pipeline:
            # Layer 2+: GuardianLens 4-phase pipeline
            pipeline_result = await run_pipeline(text_content, child_age=request.child_age)

            logger.info(
                f"[Analysis] Pipeline result: is_risky={pipeline_result['is_risky']} | "
                f"confidence={pipeline_result['confidence']:.2%} | "
                f"label={pipeline_result['risk_label']}"
            )

            logger.info(
                f"[Analysis] Pipeline: is_risky={pipeline_result['is_risky']} | "
                f"confidence={pipeline_result['confidence']:.2%} | "
                f"label={pipeline_result['risk_label']}"
            )

            if pipeline_result["is_risky"]:
                return ContentAnalysisResponse(
                    safe=False,
                    action="blocked",
                    blocked_by="guardianlens_pipeline",
                    category=pipeline_result["risk_label"],
                    confidence=pipeline_result["confidence"],
                    trigger_words=pipeline_result["trigger_words"],
                    parent_report=pipeline_result["parent_report"],
                    child_message=pipeline_result["child_message"],
                    stage=pipeline_result["stage"],
                    risk_score=pipeline_result["confidence"]
                )
            # Match pipeline's WARN_THRESHOLD of 0.15
            elif pipeline_result["confidence"] >= WARN_THRESHOLD:
                return ContentAnalysisResponse(
                    safe=True,
                    action="warn",
                    category=pipeline_result.get("risk_label"),
                    child_message=pipeline_result.get("child_message"),
                    risk_score=pipeline_result["confidence"]
                )
            elif not result["safe"] and result.get("blocked_by") == "url_keywords":
                # Low-confidence URL keyword hit: keep the bubble path alive.
                return ContentAnalysisResponse(
                    safe=True,
                    action="none",
                    ...
                    domain=domain,
                    trigger_words=pipeline_result.get("trigger_words"),
                    parent_report=pipeline_result.get("parent_report"),
                    child_message=pipeline_result.get("child_message"),
                    risk_score=pipeline_result["confidence"],
                    stage=pipeline_result["stage"],
                )
