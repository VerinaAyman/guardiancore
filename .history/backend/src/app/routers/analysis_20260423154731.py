...
async def analyze_content(
    ...
    try:
        ...
        if result["safe"] and text_content:
            # Layer 2+: GuardianLens 4-phase pipeline
            pipeline_result = await run_pipeline(text_content, child_age=request.child_age)
            logger.info(f"[Analysis] Pipeline result: is_risky={pipeline_result.get('is_risky')}, confidence={pipeline_result.get('confidence')}")

            # ✅ FIX: correct indentation — was off by 1 space, crashing the whole block
            ...
            # ✅ FIX: explicit 0.15 threshold — catches medium-risk sites like Wattpad/Tumblr
            elif pipeline_result.get("confidence", 0) > 0.15:
                return ContentAnalysisResponse(
                    safe=True,
                    action="warn",
