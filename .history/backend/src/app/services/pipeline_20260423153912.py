...
async def analyze_content(
    ...
    try:
        ...
        if result["safe"] and text_content:
            ...
            elif pipeline_result.get("confidence", 0) > 0:
                # Middle case: pipeline ran but decided safe — return score for warn bubble
                return ContentAnalysisResponse(
                    safe=True,
                    action="warn",
        ...
        logger.error(f"[Analysis] Failed, allowing (fail-open): {e}")
        return ContentAnalysisResponse(safe=True, action="none")
