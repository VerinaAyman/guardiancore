...
// ✨ Cuter owl · unified design · spring animations · Web Audio sounds
...
(function () {
  ...
  // 🟢 Safe: cheerful ascending chime — "yay!"
  function soundSafe() {
    playTone([[523,0.15,0.14],[659,0.15,0.14],[784,0.15,0.14],[1047,0.30,0.16]], 'sine');
  }
  // 🟡 Warn: soft "uh oh" — gentle, not scary
  function soundWarn() {
    playTone([[660,0.18,0.13],[520,0.18,0.13],[440,0.28,0.12]], 'triangle');
  }
  // 🔴 Block: low "bwong" — firm but not harsh
  function soundBlock() {
    playTone([[320,0.12,0.16],[240,0.16,0.16],[180,0.35,0.14]], 'sawtooth');
  }

  // ─── SVG Owl Factory — CUTER & MORE CHILD-FRIENDLY ───────────────────────
  // Big round head, huge friendly eyes, chubby cheeks, expressive brows
  function owlSVG(mood) {
    ...
    // ── Eyebrows — big, readable, expressive ──
    // warn: raised + tilted inward (worried V shape)
    // block: pinched down hard (angry ∧ shape)
    // safe: gently arched up (happy)
    const browL = mood === 'warn'  ? 'M 26 30 Q 33 24 40 28'   // worried: inner raised
                : mood === 'block' ? 'M 24 28 Q 33 34 40 29'   // frustrated: scrunched down-center
                : 'M 26 32 Q 33 28 40 31';                      // happy: gentle arch
    ...
    // ── Mouth ──
    // warn: small worried frown with corners down
    // block: big pouty frown
    // safe: wide happy smile
    const mouth = mood === 'warn'  ? 'M 43 68 Q 50 64 57 68'
                : mood === 'block' ? 'M 40 70 Q 50 63 60 70'
                : 'M 41 66 Q 50 74 59 66';

    // ── Pupils — direction conveys mood ──
    // warn: look up + together (anxious look)
    // block: look down + apart (sulky look)
    // safe: look straight (friendly)
    ...
    // ── Mood extras ──
    // warn: sweat drop (cute, not scary)
    // block: little steam puffs (frustrated, cartoony)
    // safe: small star sparkle
    const extra = mood === 'warn'
      ...
      : `<g opacity="0.9">
          <text x="76" y="26" font-size="12" fill="#fbbf24">✦</text>
        </g>`;

    // ── Eye shape — half-closed for block (pouty), wide-open for warn/safe ──
    const eyeClipBlock = mood === 'block'
      ? `<defs>
          <clipPath id="eyeClipL-${mood}">
            <rect x="18" y="42" width="30" height="22" rx="2"/>
          </clipPath>
          <clipPath id="eyeClipR-${mood}">
            <rect x="52" y="42" width="30" height="22" rx="2"/>
          </clipPath>
        </defs>`
      : '';

    const eyeClipAttrL = mood === 'block' ? `clip-path="url(#eyeClipL-${mood})"` : '';
    const eyeClipAttrR = mood === 'block' ? `clip-path="url(#eyeClipR-${mood})"` : '';

    // ── Heavy-lid line for block mood ──
  ...
  <defs>
    <radialGradient id="owlBG-${mood}" cx="50%" cy="38%" r="60%">
      <stop offset="0%" stop-color="${c.belly}"/>
      <stop offset="100%" stop-color="${c.body}"/>
    </radialGradient>
    ${mood === 'block' ? `
    <clipPath id="eyeClipL-${mood}"><rect x="18" y="42" width="30" height="22" rx="2"/></clipPath>
    <clipPath id="eyeClipR-${mood}"><rect x="52" y="42" width="30" height="22" rx="2"/></clipPath>` : ''}
  </defs>

  <!-- Wings — chubby rounded -->
  ...
  <ellipse cx="50" cy="63" rx="32" ry="32" fill="url(#owlBG-${mood})"/>

  <!-- Belly patch — big soft oval -->
  <ellipse cx="50" cy="67" rx="20" ry="24" fill="${c.belly}" opacity="0.65"/>

  <!-- Ear tufts — chunky and cute -->
  ...
  <!-- Eye whites — big round friendly eyes -->
  <circle cx="33" cy="49" r="14" fill="white" ${eyeClipAttrL}/>
  <circle cx="67" cy="49" r="14" fill="white" ${eyeClipAttrR}/>

  <!-- Iris rings — coloured -->
  <circle cx="33" cy="49" r="10" fill="${c.eye}" opacity="0.22" ${eyeClipAttrL}/>
  <circle cx="67" cy="49" r="10" fill="${c.eye}" opacity="0.22" ${eyeClipAttrR}/>

  <!-- Pupils — large & expressive -->
  <circle cx="${lPx}" cy="${lPy}" r="7" fill="${c.pupil}" ${eyeClipAttrL}/>
  <circle cx="${rPx}" cy="${rPy}" r="7" fill="${c.pupil}" ${eyeClipAttrR}/>

  <!-- Eye shine — two dots for extra cuteness -->
  <circle cx="${parseInt(lPx)+3}" cy="${parseInt(lPy)-3}" r="2.5" fill="white" ${eyeClipAttrL}/>
  <circle cx="${parseInt(lPx)-1}" cy="${parseInt(lPy)+2}" r="1.2" fill="white" opacity="0.6" ${eyeClipAttrL}/>
  <circle cx="${parseInt(rPx)+3}" cy="${parseInt(rPy)-3}" r="2.5" fill="white" ${eyeClipAttrR}/>
  <circle cx="${parseInt(rPx)-1}" cy="${parseInt(rPy)+2}" r="1.2" fill="white" opacity="0.6" ${eyeClipAttrR}/>

  <!-- Heavy lids for block mood -->
  ${heavyLids}
