...
const rawCategory = p.get("category") || "restricted";
const rawReason   = p.get("reason")   || "";
...
const humanCategory = categoryLabels[rawCategory.toLowerCase()] ||
  rawCategory.replace(/_/g, " ").replace(/\b\w/g, l => l.toUpperCase());

// ── Clean raw codes from reason text ──
const cleanReason = rawReason
  .replace(/blocked_keyword/gi, "harmful language")
  .replace(/unsafe_content/gi, "unsafe content")
  .replace(/ai_blocked/gi, "AI safety detection")
  .replace(/guardianlens_pipeline/gi, "AI safety detection");
...
document.getElementById("narrative").textContent = cleanReason || narratives[stage] || narratives[3];

// ── XAI trigger tokens — Stage 3+ only ──
if (tokens && stage >= 3) {
