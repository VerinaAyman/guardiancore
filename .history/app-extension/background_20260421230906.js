...
async function captureActivityEvent(eventType, domain, additionalData = {}) {
  try {
    ...
    const eventData = { domain, event_type: eventType, ...additionalData };
...
chrome.webNavigation.onBeforeNavigate.addListener((nav) => {
  ...
  for (const rule of rulesCache.allowlist) {
    if (matchesPattern(nav.url, rule.pattern)) {
      console.log("[Blocking] ✅ Allowed by allowlist rule:", rule.pattern);
      return;
    }
  }
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message.type === "GET_TAB_STATE") {
    ...
    // Proxy Groq API call through Service Worker to avoid CSP blocks
