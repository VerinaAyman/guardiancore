...
async function updateDynamicBlockingRules() {
  ...
  for (const rule of rulesCache.blocklist) {
    ...
    const redirectUrl = chrome.runtime.getURL("blocked.html") + `?url=${encodeURIComponent(rule.pattern || '')}&category=${encodeURIComponent(rule.category || "blocked_keyword")}&reason=${encodeURIComponent(rule.explanation || rule.category || 'Blocked by rule')}`;
