...
async function initializeAuth() {
  ...
  try {
    ...
    if (currentUser.account_type === 'child') {
      try {
        const profileResponse = await fetch(`${backendUrl}/accounts/profile`, {
          method: 'GET',
          headers: { 'Authorization': `Bearer ${gc_auth_token}` }
        });
        if (profileResponse.ok) {
          const profile = await profileResponse.json();
          currentUser.parentEmail = profile.parent_email || profile.email || '';
        }
      } catch (error) {
        console.warn('[Auth] Failed to load parent info:', error);
      }
    }
