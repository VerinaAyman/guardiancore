...
(function () {
  ...
  // ─── Config ────────────────────────────────────────────────────────────────
  // Risk is received as 0–1 from backend (e.g. 0.05 = 5%)
  const RISK_WARN_THRESHOLD    = 0.05;  // medium danger → auto-open warning
  const RISK_ESCALATE_THRESHOLD = 0.85; // high danger → escalate to parents
  ...
  function buildSystemPrompt(ctx) {
    if (!ctx) {
      return `You are Lens, a warm and smart AI safety buddy built into a parental control extension for kids aged 6–16 in Egypt. Be friendly, helpful, and keep responses to 2-3 sentences max.`;
    }
...
- Risk score: ${Math.round(ctx.risk * 100)}/100
...
Respond ONLY as Lens. No meta-commentary.`;
  ...
  function injectStyles() {
    ...
    style.textContent = `
      ...
      #lens-panel.warn-mode .lens-topbar { background: #FF7043; }
      #lens-panel.escalate-mode .lens-topbar { background: #E53935; }
      ...
      .lens-dismiss-btn:hover { background: #FF7043; color: #fff; }
  ...
  function buildPanel() {
    ...
    panel.innerHTML = `
      <div class="lens-topbar">
        ...
        <div class="lens-meta">
          ...
          <div class="lens-sub" id="lens-sub">always watching your back</div>
        </div>
        <button class="lens-close" aria-label="Close Lens" id="lens-close-btn">✕</button>
      </div>
    ...
    document.getElementById('lens-close-btn').addEventListener('click', () => {
      // If this is a warning that hasn't been dismissed yet, log it
  ...
  function togglePanel() {
    ...
    if (chatOpen) {
      ...
      // Log dismiss if warning was shown and user toggled closed
      if (currentContext && currentContext.risk >= RISK_WARN_THRESHOLD &&
          currentContext.risk < RISK_ESCALATE_THRESHOLD && !warningDismissed) {
        logDismiss();
        warningDismissed = true;
  ...
  // ─── Inject dismiss bar for warn mode ────────────────────────────────────────
  function injectDismissBar() {
    ...
    bar.innerHTML = `
      <span class="lens-dismiss-text">You can still stay on this page.</span>
      <button class="lens-dismiss-btn" id="lens-dismiss-btn">Got it, continue</button>
    `;
    // Insert between chat and input bar
    const panel = document.getElementById('lens-panel');
    ...
    document.getElementById('lens-dismiss-btn').addEventListener('click', () => {
      ...
      // Reset bubble to normal after dismiss
  ...
  async function triggerLens(ctx) {
    ...
    if (ctx.risk >= RISK_WARN_THRESHOLD) {
      ...
      setTimeout(async () => {
        ...
        setLock(false);
        // Show dismiss bar so kid can continue browsing
        injectDismissBar();
      }, 800);
    }
  }
