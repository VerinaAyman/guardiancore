...
Be warm, kind, age-appropriate. Keep answers 2-3 sentences max. Use 1 emoji. Never scary language. Encourage talking to a trusted adult if needed.`,
  ...
  // ─── Warn / info bubble ───────────────────────────────────────────────────
  function showLensBubble({ risk = 50, category = '', reason = '', domain = '', onContinue = null, onLeave = null }) {
    removeBubble();
    injectFont(); injectStyles();
    soundWarn();
    ...
    const canLeave = typeof onLeave === 'function';
    ...
    bubble.innerHTML = `
      ...
      <div class="gl-footer">
        ...
        <button class="gl-btn gl-btn-chat" id="gl-btn-chat">🦉 Let's Chat</button>
          `
          : `
        <button class="gl-btn gl-btn-dismiss" id="gl-btn-leave">Go back</button>
        <button class="gl-btn gl-btn-chat" id="gl-btn-continue">Continue anyway</button>
          `
        }
      </div>
    `;
    ...
    const leaveBtn    = document.getElementById('gl-btn-leave');
    const continueBtn = document.getElementById('gl-btn-continue');
  ...
  function init() {
    ...
    // Freeze immediately — before anything loads visually
    hidePage();

    // Start analysis immediately, no delay
    if (document.readyState === 'complete') {
      requestAnalysis(isRiskyDomain());
    } else {
      document.addEventListener('DOMContentLoaded', () => requestAnalysis(isRiskyDomain()));
    }
  }
