...
async function handleAnalyzePageRequest(message, sender) {
  ...
  try {
    ...
    // ── BLOCKED ──────────────────────────────────────────────────────────
    if (!result.safe) {
      console.log(`[Analysis] ⚠️ Unsafe: ${result.blocked_by} - ${result.category}`);
      ...
      if (sender.tab && sender.tab.id) {
        chrome.tabs.update(sender.tab.id, { url: chrome.runtime.getURL("blocked.html") + `?reason=${encodeURIComponent(result.child_message || '')}&category=${encodeURIComponent(result.category || 'Restricted content')}&url=${encodeURIComponent(url)}` });
      }
      return { received: true, blocked: true, action: 'blocked', category: result.category, rule_created: result.rule_created };
    }

    // ── WARN: medium risk — auto-open GuardianLens bubble ─────────────────
    const riskScore = result.risk_score ?? result.confidence ?? 0;
    if (riskScore >= 0.05 && sender.tab && sender.tab.id) {
      console.log(`[Lens] Triggering warning bubble — risk: ${riskScore}`);
