...
async function loadXpState() {
  try {
    ...
    ensureXpDay();
  } catch (error) { console.error("[XP] Failed to load XP state:", error); }
}
...
async function captureActivityEvent(eventType, domain, additionalData = {}) {
  ...
  } catch (error) { console.debug("[Activity] Failed to capture event:", error); }
}
...
async function loadChildRules(childId, token) {
  try {
    ...
    if (!response.ok) { console.error("[Rules] Failed to load child rules:", response.status); return; }
    ...
    console.log("[Rules] Child rules loaded:", { total: rules.length, enabled: enabledRules.length, allowlist: rulesCache.allowlist.length, blocklist: rulesCache.blocklist.length, time_window: rulesCache.time_window.length });
    console.log("[Rules] Blocklist rules:", rulesCache.blocklist);
    await updateDynamicBlockingRules();
  } catch (error) { console.error("[Rules] Failed to load child rules:", error); }
}

async function fetchRules() {
  if (currentUser && currentUser.account_type === 'child') await loadChildRules(currentUser.user_id, currentUser.token);
}
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.type === "GET_TAB_STATE") {
    ...
    chrome.storage.sync.get('lens_groq_key', async (res) => {
      ...
      if (!key) { sendResponse({ reply: "Hey! Looks like GuardianLens isn't fully set up yet. Ask your parent to add the Lens API key in the extension settings 🦉" }); return; }
      try {
    ...
    console.log(`[Lens] Warning dismissed on ${message.domain} (risk: ${message.risk}, category: ${message.category})`);
    const domain = message.domain || extractDomain(message.url) || 'unknown';
    captureActivityEvent('warning_dismissed', domain, {
      risk_score: message.risk,
    ...
    // ── High-risk page: notify parent via ALL channels ──────────────────
    console.log(`[Lens] 🚨 ESCALATION triggered for ${message.domain}`);
    const domain      = message.domain || extractDomain(message.url) || 'unknown';
    ...
    // 1. Backend notification → email + dashboard feed
    notifyParent({
      url: escalUrl,
      category: escalCat,
      parent_report: `GuardianLens flagged a high-risk page. ${childName} visited content categorised as "${escalCat}" on ${domain}. Please review their activity dashboard immediately.`,
      trigger_words: message.trigger_words || []
    });

    // 2. Activity log entry (shows in parent dashboard feed)
    captureActivityEvent('escalation', domain, {
      category: escalCat,
      url: escalUrl,
      risk_level: 'high',
    ...
    // 3. Chrome push notification (visible even when browser is minimised)
    chrome.notifications?.create?.(`lens-escalate-${Date.now()}`, {
      type: 'basic',
    ...
    console.log(`[Lens] ✅ Parent notified via backend + push for: ${domain}`);
    sendResponse({ ok: true });
  }
});
...
// ========== INTELLIGENT CONTENT CLASSIFICATION ==========
// REPLACE the entire handleAnalyzePageRequest function in your background.js with this
...
async function handleAnalyzePageRequest(message, sender) {
  ...
  console.log("[Analysis] URL:", url, "| Text length:", text?.length || 0);
  ...
  for (const rule of rulesCache.allowlist) {
    if (matchesPattern(url, rule.pattern)) {
      console.log(`[Analysis] ✅ ${domain} is ALLOWLISTED`);
  ...
  for (const rule of rulesCache.blocklist) {
    if (matchesPattern(url, rule.pattern)) {
      console.log(`[Analysis] 🚫 ${domain} is already BLOCKLISTED`);
  ...
  try {
    ...
    const response = await fetch(`${backendUrl}/analyze/content`, {
      ...
      body: JSON.stringify({
        ...
        child_age: 13,
        // Tell the backend to be sensitive to slang, intent, and context
        sensitivity: 'high',
        check_slang: true,
        check_intent: true
      })
    ...
    const isSafe = result.safe !== false && confidence < 0.30;

    console.log(`[Analysis] Confidence: ${confidence} | safe flag: ${result.safe} | computed safe: ${isSafe}`);

    // ── HARD BLOCK: confidence >= 0.65 or backend explicitly blocked ──────
    // content.js handles the redirect — background just logs and notifies
    if (!isSafe && (confidence >= 0.65 || result.action === 'blocked')) {
      console.log(`[Analysis] 🚫 BLOCK — confidence: ${confidence} | category: ${result.category}`);
      ...
      // Return blocked — content.js does the actual redirect via silentBlock()
      return {
        received: true,
        safe: false,
        action: 'blocked',
        category: result.category || 'Restricted content',
    ...
    // ── WARN: confidence >= 0.30 and < 0.65 — show side bubble ───────────
    if (!isSafe && confidence >= 0.30) {
      console.log(`[Analysis] ⚠️ WARN — confidence: ${confidence} | category: ${result.category}`);

      await notifyParent({
        url,
        category: result.category || 'risky_content',
        parent_report: result.parent_report || `Risky content detected on ${domain} (confidence: ${Math.round(confidence * 100)}%)`,
        trigger_words: result.trigger_words || []
      });

      // Return warn — content.js fires the LENS_TRIGGER bubble
      return {
        received: true,
        safe: false,
        action: 'warn',
        category: result.category || 'Potentially inappropriate',
        child_message: result.child_message || 'This page has some content worth talking about.',
        confidence,
    ...
    // Still trigger low-risk bubble if confidence is between 0.10 and 0.30
    if (confidence >= 0.10 && sender.tab && sender.tab.id) {
      console.log(`[Lens] Low-risk bubble — confidence: ${confidence}`);
