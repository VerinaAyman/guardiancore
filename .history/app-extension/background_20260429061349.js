...
async function ensureDefaultConfig() {
  try {
    ...
    if (!gl_config_initialized) {
      const defaults = {};
      if (!gc_backend_url) {
        defaults.gc_backend_url = 'http://localhost:8000'; // backend URL unchanged
...
const BLOCKED_CATEGORIES = [
  'Adult Content', 'adult_content', 'adult content',
  'Gambling', 'gambling',
  'Drugs', 'drugs',
...
async function updateDynamicBlockingRules() {
  ...
  let ruleId = 1;
  for (const rule of rulesCache.blocklist) {
    // Skip WARN_CATEGORIES — only add BLOCKED_CATEGORIES to DNR
     if (WARN_CATEGORIES.some(c => c.toLowerCase() === (rule.category || '').toLowerCase())) {
      console.log(`[DNR] Skipping ${rule.pattern} (category: ${rule.category} — AI-only)`);
      continue;
    }
    const pattern = rule.pattern.toLowerCase();
    // Skip single-word keyword filters (no dots, no slashes) — not domain-based
    if (!pattern.includes('.') && !pattern.includes('/')) {
      console.log(`[DNR] Skipping ${rule.pattern} (keyword filter — AI-only)`);
      continue;
    }
    const HARD_BLOCK_ONLY = ['adult_content', 'adult content', 'gambling', 'malware', 'phishing'];
    const isDangerous = HARD_BLOCK_ONLY.some(c => c.toLowerCase() === (rule.category || '').toLowerCase());
    if (!isDangerous) {
      console.log(`[DNR] Skipping ${rule.pattern} (category: ${rule.category} — leaving to AI)`);
      continue;
    }
...
chrome.webNavigation.onBeforeNavigate.addListener((nav) => {
  ...
  // Send a message to content script to start collecting page text early, before full load
...
async function handleAnalyzePageRequest(message, sender) {
  ...
  try {
    ...
    // ── CATEGORY-BASED CLASSIFICATION MAPPING ──────────────────────────────
    // Map Groq-returned category to action (BLOCKED, WARN, or SAFE)
    let action = 'safe';  // default
    const catLower = (returnedCategory || '').toLowerCase().trim();
    if (BLOCKED_CATEGORIES.some(c => c.toLowerCase() === catLower)) {
      action = 'blocked';
    } else if (WARN_CATEGORIES.some(c => c.toLowerCase() === catLower)) {
      action = 'warn';
    ...
    console.log(`[GL-DEBUG] category="${returnedCategory}" confidence=${confidence} action=${action}`);

    console.log(`[Analysis] Category: "${returnedCategory}" → Action: ${action} | Confidence: ${confidence}`);
    ...
    if (action === 'blocked') {
      ...
      awardXp({ trackers: 0, csp: false, cors: false, blocked: true, violation: true });
      await notifyParent({ url, category: returnedCategory || 'ai_blocked', parent_report: result.parent_report || '', trigger_words: result.trigger_words || [] });
      try {
        if (sender?.tab?.id) {
      ...
      return { received: true, safe: false, action: 'blocked', category: returnedCategory || 'Restricted content', child_message: result.child_message || '', confidence, stage: result.stage || 3 };
    }

    // ── WARN ──────────────────────────────────────────────────────────────
    if (action === 'warn') {
      console.log(`[Analysis] ⚠️ WARN — category: ${returnedCategory}, confidence: ${confidence}`);
      await notifyParent({ url, category: returnedCategory || 'risky_content', parent_report: result.parent_report || `Content on ${domain} (${returnedCategory})`, trigger_words: result.trigger_words || [] });
      try {
  if (sender?.tab?.id) {
    await chrome.scripting.executeScript({
      target: { tabId: sender.tab.id },
      func: (u, cat, rsn) => { window.gl_warning_data = { url: u, category: cat, reason: rsn }; },
      args: [url, returnedCategory || 'Potentially inappropriate', result.child_message || result.parent_report || '']
    });
    await chrome.scripting.executeScript({
      target: { tabId: sender.tab.id },
...
} catch (e) { console.warn('[Analysis] Failed to inject warning overlay:', e); }
    ...
    if (confidence >= 0.10 && sender.tab && sender.tab.id) {
      chrome.tabs.sendMessage(sender.tab.id, {
        ...
        risk: confidence * 100,
        domain,
        category: returnedCategory || "Worth a chat",
