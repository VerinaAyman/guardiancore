...
const BLOCKED_CATEGORIES = ['Adult Content', 'Gambling', 'Drugs', 'Hate Speech', 'Malware', 'Phishing'];
const WARN_CATEGORIES    = ['Social Media', 'Gaming', 'Forums', 'Video', 'Anime'];
...
async function updateDynamicBlockingRules() {
  ...
  for (const rule of rulesCache.blocklist) {
    ...
    if (WARN_CATEGORIES.includes(rule.category)) {
...
async function handleAnalyzePageRequest(message, sender) {
  ...
  const EDUCATIONAL_WHITELIST = ['wikipedia.org', 'khanacademy.org', 'bbc.co.uk', 'nationalgeographic.com', 'britannica.com', 'google.com', 'youtube.com'];
  for (const whitelistedDomain of EDUCATIONAL_WHITELIST) {
    if (domain === whitelistedDomain || domain.endsWith('.' + whitelistedDomain)) {
      console.log(`[Analysis] ✅ ${domain} WHITELISTED (educational)`);
      return { received: true, safe: true, skipped: true, reason: "educational_whitelist" };
    }
  }
  ...
  try {
    ...
    if (BLOCKED_CATEGORIES.includes(returnedCategory)) {
      action = 'blocked';
    } else if (WARN_CATEGORIES.includes(returnedCategory)) {
      action = 'warn';
    }
    // else remains 'safe'
    ...
    if (action === 'warn') {
      ...
      try {
        if (sender?.tab?.id) {
          const warningUrl = chrome.runtime.getURL('warning.html') + `?url=${encodeURIComponent(url)}&category=${encodeURIComponent(returnedCategory || 'Potentially inappropriate')}&reason=${encodeURIComponent(result.child_message || result.parent_report || '')}&proceed=${encodeURIComponent(url)}`;
