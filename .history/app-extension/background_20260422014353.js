...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.type === "GET_TAB_STATE") {
    ...
    // Proxy Groq call through Service Worker to avoid CSP blocks on content pages
    chrome.storage.sync.get('lens_groq_key', async (res) => {
      const key = res.lens_groq_key;
      if (!key) { sendResponse({ reply: "Hey! Looks like Lens isn't fully set up yet. Ask your parent to add the Lens API key in the extension settings 🛡" }); return; }
      try {
    ...
    // Kid saw the warning and chose to stay — log it
    ...
    // High-risk page — notify parent immediately
    console.log(`[Lens] ESCALATION triggered for ${message.domain}`);
    const domain = message.domain || extractDomain(message.url) || 'unknown';
    notifyParent({
      url: message.url || `https://${domain}`,
      category: message.category || 'high_risk',
      parent_report: `Child visited high-risk content on ${domain}`,
      trigger_words: []
    });
    captureActivityEvent('escalation', domain, { category: message.category, url: message.url });
    sendResponse({ ok: true });
  }
});
...
async function handleAnalyzePageRequest(message, sender) {
  ...
  try {
    ...
    // ── WARN: medium risk — auto-open Lens bubble ─────────────────────
