...
async function scheduleTokenRefresh() {
  ...
  const msUntilRefresh = expiry - Date.now() - (60 * 60 * 1000); // 1hr before expiry
  if (msUntilRefresh <= 0) {
...
const ANALYSIS_COOLDOWN_MS = 0; // NOO 5 minute cooldown per domain (only for domains not in allow/blocklist)
  ...
  try {
    ...
    if (!result.safe) {
      ...
      if (sender.tab && sender.tab.id) {
        // parent_report intentionally excluded — sent by backend email only (Paper 26)
        const tokens = Array.isArray(result.trigger_words)
        ...
        const blockUrl = chrome.runtime.getURL("blocked.html") +
          ...
          `&stage=${encodeURIComponent(result.stage || 3)}`;

        chrome.tabs.update(sender.tab.id, { url: blockUrl });

        // Fire-and-forget: ask backend to email the parent report privately
        if (result.parent_report) {
          fetch(`http://localhost:8000/notify/parent-report`, {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
              "Authorization": `Bearer ${currentUser.token}`
          ...
          }).catch(e => console.warn("[Notify] Parent email failed (non-blocking):", e));
        }
      }
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  if (msg.type !== 'LENS_GROQ_REQUEST') return false;
  ...
  chrome.storage.sync.get('lens_groq_key', async (res) => {
    ...
    try {
      ...
      const reply = data.choices?.[0]?.message?.content?.trim() || "I had a little hiccup — try again in a sec!";
      sendResponse({ reply });
    } catch (e) {
