...
(function () {
  ...
  // ─── Design Tokens (mirror background.js / popup) ─────────────────────────
  const GL = {
    // colours
    safe:    '#22c55e',
    warn:    '#f59e0b',
  ...
  function injectStyles() {
    ...
    style.textContent = `
      ...
      .gl-bar-warn  { background: linear-gradient(90deg, var(--gl-warn), #fbbf24, var(--gl-warn)); background-size:200%; animation: gl-progress-fill 0.6s ease both; }
      ...
      .gl-shield-ring.active {
        animation: gl-pulse-ring 1.4s cubic-bezier(0,0,0.2,1) infinite;
      }
      ...
      /* ── Safe chip (small bottom-right tag) ── */
      #gl-safe-chip {
        position: fixed;
        bottom: 20px;
        right: 20px;
      ...
      #gl-safe-chip.gl-exit {
        animation: gl-spring-out 0.3s ease both;
      }
    `;
  ...
  function showLensBubble({ risk = 50, category = '', reason = '', domain = '' }) {
    ...
    // Sparkles on high-risk
    if (isHighRisk) {
      setTimeout(() => burstSparkles(document.getElementById('gl-shield-wrap')), 300);
    }

    // Dismiss
    document.getElementById('gl-btn-dismiss')?.addEventListener('click', () => {
      removeBubble();
    ...
    // Chat (placeholder — open popup or tab)
    document.getElementById('gl-btn-chat')?.addEventListener('click', () => {
      removeBubble();
      chrome.runtime.sendMessage({ type: 'LENS_OPEN_CHAT', domain, category }).catch(() => {});
    });
  }
  ...
  function silentBlock(category, reason, url) {
    ...
    }, 180); // tiny delay so sound can fire
  }
  ...
  function handleAnalysisResponse(response, wasHidden) {
    ...
    if (wasHidden) showPage();
    // Only show safe chip if confidence was above 0.10 (near-miss)
  ...
  // ─── LENS_TRIGGER listener (from background.js) ───────────────────────────
