...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.type === "GET_TAB_STATE") {
    ...
    chrome.storage.local.remove(['gc_auth_token','gc_user_id','gc_account_type','gc_username','gc_email','gl_pin','gl_recovery_codes','gl_pin_verified']).then(async () => {
      const tabs = await chrome.tabs.query({ url: chrome.runtime.getURL('login.html') });
      if (tabs.length === 0) chrome.tabs.create({ url: chrome.runtime.getURL('login.html') });
    ...
    try {
      if (!currentUser) return;
      const msg = message;
      fetch('https://guardiancore.onrender.com/notify/block-alert', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + currentUser.token },
        body: JSON.stringify({ blocked_url: msg.url, reason: msg.reason, child_username: currentUser.username, timestamp: new Date().toISOString() })
      }).then((res) => {
        if (res.ok) console.log('[GuardianLens] Parent alert sent');
        else console.warn('[GuardianLens] Parent alert failed');
      }).catch((err) => {
        console.warn('[GuardianLens] Parent alert failed', err);
      });
