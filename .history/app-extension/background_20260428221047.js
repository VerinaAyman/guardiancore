...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  } else if (message.type === 'CLAUDE_REQUEST') {
    const payload = message.payload || {};
    chrome.storage.sync.get('anthropic_api_key', (res) => {
      const key = res?.anthropic_api_key;
      if (!key) { sendResponse({ reply: 'No API key configured' }); return; }
      ...
      .then(r => r.json())
      .then(data => {
        const reply = data.content?.find?.(b => b.type === 'text')?.text || data.completion?.content || 'No response';
        sendResponse({ reply });
      })
      .catch(() => sendResponse({ reply: 'Connection error!' }));
    });
    return true;
