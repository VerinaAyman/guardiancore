...
async function handleAnalyzePageRequest(message, sender) {
  ...
  // TEMP TEST: override text with toxic content
  const text_override = "I hate you so much I want to kill you and destroy everything";
  message.text = text_override;
  ...
  try {
    ...
    if (!result.safe) {
      ...
      if (sender.tab && sender.tab.id) {
        const blockUrl = chrome.runtime.getURL("blocked.html") +
        `?reason=${encodeURIComponent(result.child_message || 'AI Detection: ' + (result.category || 'Unsafe content'))}` +
        `&category=${encodeURIComponent(result.category || 'ai_blocked')}` +
        `&url=${encodeURIComponent(url)}` +
        `&rule_created=${result.rule_created ? 'true' : 'false'}` +
        `&parent_report=${encodeURIComponent(result.parent_report || '')}` +
