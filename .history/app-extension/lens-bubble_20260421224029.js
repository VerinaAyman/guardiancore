...
(function () {
  ...
  // ─── Config ────────────────────────────────────────────────────────────────
  const RISK_GUIDE_THRESHOLD = 40;
  const RISK_ESCALATE_THRESHOLD = 85;
...
Respond ONLY as Lens. No meta-commentary.`;
  ...
  // ─── Groq API call — routed through background.js to avoid CSP blocks ───────
  async function askLens(userMessage) {
    if (userMessage) {
      chatHistory.push({ role: 'user', content: userMessage });
    }

    console.log('[Lens] Sending message, context:', currentContext, 'history:', JSON.stringify(chatHistory));
    ...
    return new Promise((resolve) => {
      chrome.runtime.sendMessage({
        ...
        console.log('[Lens] Response received:', JSON.stringify(response), 'lastError:', chrome.runtime.lastError?.message);
